...
::: {}
[Gregory Clark](http://faculty.econ.ucdavis.edu/faculty/gclark/), [Neil Cummins](http://neilcummins.com/), and I have created new occupational status indices for England and Wales. The first index uses the RCII method used by [HISCAM](http://www.camsis.stir.ac.uk/hiscam/) and is avaliable for [HISCO codes](https://historyofwork.iisg.nl/) and our own occupational coding scheme. The second index is the first principle component of several measures of socioeconomic status and is avaliable for our own occupational coding scheme. [HISCO scores](http://neilcummins.com/hisco-rcii-scores.csv). Our own occupational coding scheme [scores](http://neilcummins.com/AppendixA2.csv).

To compute the RCII scores, I wrote some [R code](./code/rc2.zip) that is much faster than the previously existing Stata code (using the amazing [fixest](https://lrberge.github.io/fixest/) package!)
